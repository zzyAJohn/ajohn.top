...
Channel 是对一个文件描述符（fd）上感兴趣事件的封装：记录想监听哪些事件（读/写/无），保存事件发生后要调用的回调函数，并把这些变更告诉 EventLoop/Poller。当 Poller 检测到 fd 有事件时，EventLoop 会调用 `Channel::handleEvent`，由 Channel 去触发对应的回调。
