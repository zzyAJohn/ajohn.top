...
1. [2144. 打折购买糖果的最小开销](https://leetcode.cn/problems/minimum-cost-of-buying-candies-with-discount/description/)
    ...
    ::: tip 
    思路：**从最大开始贪心，三个一组序列。** 要求花费最小，我们需要赠送的糖果价值尽可能地大，可以先从大到小排序，然后每三个为一组012，购买第一个和第二个，赠送第三个（即余数为2的）。
    :::
...
2. [561. 数组拆分](https://leetcode.cn/problems/array-partition/description/)
    ...
    ::: tip 
    思路：**从最小开始贪心，两个一组序列。** 要求最小值的和最大，很明显，最小值的两个数差距越小最后的和越大，因此可以排序后两个一组取最小值然后求和即可。
    :::
...
3. [1877. 数组中最大数对和的最小值](https://leetcode.cn/problems/minimize-maximum-pair-sum-in-array/description/)
    ...
    ::: tip 
    思路：**从最小开始贪心，两个一组序列。** 要求最大数对和最小，和上一题不同，这题我们要保证数对和之间的差距不大，这样求出的数对和会比较小，因此使用两个指针分别指向起始和末尾，都往中间走，这样就可以保证求出的数对和比较均匀。
    :::
